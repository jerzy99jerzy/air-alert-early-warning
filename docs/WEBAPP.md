...
| `strike_tally` | From 0.56.0.0, when this producer has polled `@kpszsu`: the latest night the Ukrainian Air Force summarised, as `night`, `as_of`, `posted_at`, `shape`, `check`, `downed_total`, `downed[]`, `launched_total`, `launched[]`, `launched_sum`, `source_url`, `read_at` and `stale_error` (D-056) | Absent before the channel was ever polled; `null` when polled and no readable night is stored, or when composing it failed; otherwise an object. **Never an empty object**, which would render as a night on which nothing was launched. `check` is `ok` or `flagged`; on `flagged` only `downed_total` carries a figure and `downed`, `launched`, `launched_total` and `launched_sum` are `null`, because their headline is theirs and our reading of a night that did not add up is not. No age ceiling, as for `pl_airspace`: `night` beside `read_at` is what says whether this is the latest |
| `strike_history` | From 0.57.0.0, published whenever `strike_tally` is: `read_at`, `as_of_date`, `latest_night`, `windows[]` over 7, 30 and 90 days and `nights[]`, one point per night across the longest window, oldest first (D-057) | The same four states as `strike_tally`, decided on the same two reads, and composed under its own guard: a fold over ninety nights that fails sends this key `null` and leaves the night standing. Each point is `night`, `read`, `downed`, `launched` and `check`, and **the series is dense**: one entry per calendar night of the longest window, whether or not a summary was read for it, because a series listing only the nights it has would draw four columns across seven days and look complete. `read` false is a night nobody read; `read` true with `downed` null is a night read that gave no figure, and the two are different claims about us. **Either way the column is blank and never a zero.** `launched` is their own inventory where they gave one and the guarded sum otherwise, and is `null` on a flagged night for the reason the night's own block withholds it |
| `strike_history.windows[]` | `days`, `start`, `end`, `nights_read`, and `downed` and `launched` each as `sum`, `nights`, `complete`, `max`, `max_night` | **`sum` is an addition across nights and `nights` is what makes it readable**: the count of nights that went into it, against `days` of calendar. `complete` false is the consumer's instruction to write `at least`. An empty window carries `sum: null` and never 0, which would say the Air Force reported nothing shot down. The window ends on today's Kyiv date when a reading for it is held and on yesterday's when it is not, so the night in progress is not counted as a night that went missing |
| `strike_tally.downed[]`, `.launched[]` | One object per weapon class as the summary names it: `class` (an open vocabulary: `drone`, `banderol`, `ballistic`, `anti_ship`, `anti_radar`, `cruise`, `sam`, `air_launched`, `kab`, `other`), `type_text` as `text`, `count`, `approx`, `derived` | **`count` null is a count they did not publish, and renders as not given, never as 0.** `derived` true marks the one piece of arithmetic allowed: a single unnumbered downed item closed by their own total. `launched_sum` is computed by the producer and is `null` unless the list is non-empty and every count is known, so a consumer never adds up a list to reach a total they did not state |
