...
```
Document:  docs/THREAT-MODEL.md, version 2.4 / 2026-09-10
...
| MT15 | Two runs against one output directory double the request rate against a service whose tolerance was measured over a single burst of twenty. The lock that prevents this was a pid written to a file and a liveness test on that pid, and a pid means nothing outside the namespace that issued it: a second container on the same mounted volume finds the number absent, calls the lock stale and takes it. **Reproduced 2026-09-10** rather than reasoned about - holder at pid 483 in the host namespace, challenger under `unshare --pid --fork`, lock taken (T26) | Exclusion is `flock` on a descriptor against the inode, so two containers on one volume contend in the kernel and no pid is consulted. The liveness heuristic is removed rather than left unused, and a test fails if `os.kill` returns to the module | `test_t26_directory_lock.py::test_a_dead_pid_in_the_file_does_not_release_a_held_lock`, and `::test_a_second_pid_namespace_cannot_take_the_lock` where `unshare` runs |
