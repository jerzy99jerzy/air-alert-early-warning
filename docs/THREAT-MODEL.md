...
```
Document:  docs/THREAT-MODEL.md, version 2.7 / 2026-09-22
...
| MT7 | Hostile input converts into an outage: a malformed payload crashes the collector during the window that matters | Contract: `ThreatSource.poll` absorbs every content failure and reports it. Measured on the fixture, Telegram and API adapters. The last of those was the sentence's own falsification: it said the API adapters did not exist while `ukrainealarm_source.py` had been the primary source since D-040 | harness A7 and A9, `test_telegram.py::test_hostile_bodies_do_not_raise` |
...
| MT9 | Correlated upstream failure. Both feeds draw on the same upstream alerting chain, so two-source agreement is not independent confirmation. Sharpened on 2026-08-29, when the channel stopped publishing and the API kept going: the two share an upstream but not a delivery path, which is exactly the failure two feeds do protect against and the only one | **Accepted, named.** Two sources protect against transport failure only. Trigger to reopen: either service moving to its own acquisition, or the ADS-B channel landing as a genuinely independent observation | not testable in-tree; recorded in `docs/DECISIONS.md` D-010 |
...
| MT12 | **Modelled, not observed.** Every production reading of the skipped counter is `0` or `unknown`, and no overflow has ever been measured `[measured 2026-09-10, from `docs/DEPLOYMENT.md`]`; the burst counts that look larger than a page are events, not messages, because one message names several areas (T37). The row stands on arithmetic rather than on an incident, which is the right reason to keep it and a fact the row did not carry until 0.54.6.0. The page is a window of roughly twenty messages. During a mass alert the channel emits more than that between two polls, and the skipped messages leave no trace, so an overflow reads as a quiet channel | Post ids are compared across polls and the skipped count is reported. Where it cannot be measured, on a first poll or a page without ids, it is reported as unknown rather than zero | harness A11, `test_sprint5.py::test_f27_a_skipped_window_is_counted` |
...
| MT15 | **Latent, and the precondition is stated because this repository does not currently meet it.** Two runs against one output directory double the request rate against a service whose tolerance was measured over a single burst of twenty. The lock preventing that was a pid written to a file and a liveness test on it, and a pid means nothing outside the namespace that issued it. **MAVO runs zero containers** (`docs/DEPLOYMENT.md` section 8), and within one namespace the old check worked: a live holder was found alive and the second run refused. The hole was reachable only across namespaces, so the exposure on the deployment that exists was nil, and the row is here because the control was false rather than because harm occurred. Reproduced 2026-09-10 as two processes in separate pid namespaces on one directory - the mechanism, not two containers on a mounted volume, which has still never been run (T26) | Exclusion is `flock` on a descriptor against the inode, so contention is the kernel's and no pid is consulted. The liveness heuristic is removed rather than left unused, and a test fails if `os.kill` returns to the module | `test_t26_directory_lock.py::test_a_dead_pid_in_the_file_does_not_release_a_held_lock`, and `::test_a_second_pid_namespace_cannot_take_the_lock` where `unshare` runs |
| MT16 | An all-clear speaks for the areas its own message says are still under alert. The channel writes the cleared area as a tag and the areas where the alert continues as prose after a marker, so a reader of tags alone emitted an all-clear and nothing else, the system going quiet about a place its source had just called dangerous. **Numbered at 0.56.0.0 (F186)**: the harness has guarded it since sprint 8 under the label MT15, which in this table was already the directory lock | The continuation areas become events of their own with `AreaRole.CONTINUATION`, carried through the store to `state.json` (T37, D-024) | harness A14 |
| MT17 | A night the reader failed to read renders as a launched total of zero. `all(...)` is true of an empty list, so "sum when every item carries a count" is satisfied by a night with no launched items at all, and the total that comes out is a figure the Air Force never published, under their name. Reader v3 stored the night of 1 August exactly so and reported it consistent (D-056, decision 9) | `launched_sum` is None for an empty list as well as for a list with an unknown count, computed once by the producer; a night whose checks failed publishes their headline alone, so no list of ours reaches a reader beside it | harness A15, `tests/test_strike.py` |
