...
| | |
| --- | --- |
| Installed | `air-alert-early-warning 0.54.8.0`, **installed 2026-09-10 09:53 UTC** `[measured 2026-09-18, from the operator's terminal]`: `/opt/mavo/venv/bin/mavo --version` printed `mavo 0.54.8.0`, and the installed package's RECORD file was last written 2026-09-10 09:53:44 UTC. **This row named 0.54.2.0 from 0.54.3.0 to 0.55.0.0**, and the host had moved to 0.54.8.0 the same morning (F170). Previously: `air-alert-early-warning 0.54.2.0`, installed 2026-09-10 08:02 UTC `[measured]`, wheel sha256 `de5dd320…c06684`, built from a worktree of tag `v0.54.2.0`, with `mavo/latency.py` as its content discriminator: absent before, and after it carrying `def _summary_of` once and `kind_events` twice. Before that: `air-alert-early-warning 0.53.4.0`, `/opt/mavo/venv`, python3.11 `[measured 2026-09-09 11:20 UTC]` |
...
**Held, and not to be run alone** (F173, F174). The hold this section carried
at 0.55.0.1, F169, is closed at 0.55.1.0: an all-clear now ends the alert it
names. What holds it now was found by the review of 0.55.0.1: a well-formed
document that is not the feed's list is published as an empty list, which
reads as calm (F173), and an airspace body the writer cannot encode stops
`state.json` for the Ukrainian picture as well as the Polish keys (F174). The
steps below stay the plan for the release that repairs both.
