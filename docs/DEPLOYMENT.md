...
| | |
| --- | --- |
| Installed | `air-alert-early-warning 0.53.2.0`, `/opt/mavo/venv`, python3.11 `[reported, the 2026-09-08 late session's record: `mavo.__version__` read from the venv interpreter on the host at 22:08 UTC; this revision was written from that record and not from a fresh host read]` |
| Installed at | **2026-09-08 22:08 UTC** `[reported, same record]`. Baseline before the install: `grep -c api_level` over the installed `mavo/sources/ukrainealarm_source.py` read **0**, the count for after was fixed from the tree at **2** before the host was read (lines 380 and 381), and the host read 2 after `pip`. That order - the number from the tree first, the host second - is the rule the 0.53.1.0 record broke by writing "present after" |
| Wheel | `air_alert_early_warning-0.53.2.0-py3-none-any.whl`, sha256 `8f5cc1de…fb41d7`, 184,510 B, built with `python3 -m build --wheel` from a worktree of tag `v0.53.2.0`, moved by base64 over the ssh control channel to a `.partial` name, `sha256sum` on both sides equal, renamed, then `sudo /opt/mavo/venv/bin/pip install --no-index --no-deps --force-reinstall`. The wheel stayed in the operator's home directory on the host, which P6 counts |
| Point of return | `[unknown]` for this install: no store copy was taken before `pip`. The install did not touch the schema, so the risk was nil, but the record cannot say so on its own. **The next release that touches the store's schema takes a copy first, or does not proceed**; this row is where the rule is stated, and for such a release it names the copy and its digest or the release did not happen. The previous return point on the host is still `events.pre-0.52.0.0`, 27,283,456 B, sha256 `641611e7…a8a55d` |
| Snapshot age at timer stop | **56 s** (file mtime 1788905278 against the host clock at 1788905334), under the 360 s ceiling, so every current key was in `previous` and the first cycle under the new code cleared what had ended rather than withholding a cycle |
...
| Collection gap | one cycle: last cycle under 0.53.1.0 at 22:07:58, first under 0.53.2.0 at 22:09:59, on the timer's own 121 s cadence |
| Discriminators | on the installed source and on the recap, not on the version string: `api_level` in `mavo/sources/ukrainealarm_source.py` (0 before, 2 after, the 2 fixed from the tree before the read), and `unknown_keys=` in the recap, a key no release before 0.53.2.0 prints |
| First post-install poll | 22:09:59 UTC: `active=0 cleared=0 unresolved=5 declined=1 unparsed=0 informational=0 unmapped_types=0 unknown_keys=0`, snapshot `fresh(121s)`, latency about a tenth of a second; `Result=success`, `ExecMainStatus=0`, timer `active` |
| D-050 on the wire | `[measured 23:02 UTC that session]` ten ACTIVE rows written after the install and all ten carrying `api_level`; every one `Yellow`, every `api_level_at` equal to the row's `began` to the microsecond. The level on the row is the level at the episode's start and is never rewritten (P2): an alert that went red after opening still reads `Yellow` in the store. The release that first shows a level to a reader must read the level from a stream that records changes, not from this field. The F148 window of known error stays closed at the 0.53.1.0 install, 2026-09-06 to 2026-09-08 20:26 UTC |
...
| --- | --- | --- |
| 0.53.2.0 | 2026-09-08 22:08 UTC; baseline `grep -c api_level` = 0, fixed from the tree as 2, read 2 after; first cycle under it 22:09:59 | **current**; D-050, the capture half. Snapshot 56 s old at timer stop, one-cycle gap, no return point taken (the schema did not move) |
...
and is why the flag is optional. The host runs 0.53.2.0 as this is written
[reported, the 2026-09-08 late session], which carries the flag; no unit on the host
