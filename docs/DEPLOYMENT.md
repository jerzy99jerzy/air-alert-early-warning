...
| `main` | 0.55.2.2 |
| Behind by | **two** releases: 0.55.2.2 records the reading in the row above and 0.55.2.1 rewrites both editions of FEED-SPEC; neither changes anything under `mavo/` but the version string, so this install is bookkeeping rather than a repair. What each outstanding release contained is in `CHANGELOG.md` and is no longer restated here, because the releases this row used to list as outstanding are installed. Superseded rows, kept for the record: at 0.55.2.1 this row read **five** releases and at 0.55.2.0 **four**, both counted from an `Installed` row naming 0.54.8.0 while the host had run 0.55.2.0 since 2026-09-19; at 0.55.1.0 it read **three**, held on F173 and F174; at 0.55.0.1 **two**, held on F169; at 0.55.0.0 **7**, counted from an `Installed` row naming 0.54.2.0 while the host already ran 0.54.8.0 (F170) |
