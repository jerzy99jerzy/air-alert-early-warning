...
| `main` | 0.51.0.1 |
| Behind by | **three** releases, 0.50.0.0, 0.51.0.0 and 0.51.0.1, and still none touches package code beyond the version string: the wheel would differ from the installed one in nothing a reader can see. The previous revision of this row named a third such release as the point at which it should stop repeating itself, and this is that release. The judgement therefore changes from install-at-convenience to install-next: the next act on this host is bringing it to 0.51.0.1, before any further release rides on the deferral |
