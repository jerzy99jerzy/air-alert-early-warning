...
**Tier 3, 13 open:** [T1](#t1-request-the-alertsinua-api-token), [T3](#t3-resolve-r2-which-currently-adds-nothing), [T5](#t5-rolling-feed-latency-drift-detection), [T41](#t41-prototype-the-push-interface-and-compare-it-against-polling), [T10](#t10-find-a-history-source-deep-enough-to-calibrate-on), [T14](#t14-second-signal-type-for-the-drone-regime), [T28](#t28-the-crossing-event-list-dated-and-sourced), [T56](#t56-is-there-an-alert-feed-for-the-romanian-border-and-the-baltics), [T57](#t57-a-week-of-the-picture-statistics-a-reader-can-open), [T58](#t58-traffic-and-road-conditions-near-the-border-refused-as-posed), [T70](#t70-which-side-drops-the-packets), [T74](#t74-the-delivery-timer-describes-a-cadence-it-stopped-running), [T88](#t88-the-host-reading-as-a-mavo-subcommand)
...
*Rewritten at 0.50.0.0, and the version is in this sentence on purpose.* The
...
its successor was written at 0.37.0.0 and left standing for thirteen more.
...
power and the rate-limit one reopened (T39), and a retry in the transport that
has yet to be observed doing its job (T69). All of it is repairs and
instruments, none of it is sprint work, and the paragraph above about not
counting those as progress applies to this week most of all.
...
Status: `done` at 0.39.1.0 [tier 3]
...
**Closed at 0.39.1.0.** `tools/adapter_lint.py` is a gate step. It reports a
module under `mavo/sources/` that parses external bytes and has no test module
exercising hostile input. Keyed on *parsing*, not on `ThreatSource`: `rso.py`
implements no protocol and parses a third party's JSON, so a protocol-shaped
rule would have exempted the newest parser in the tree. Three adapters pass,
two modules are exempt with written reasons, and the check is verified red
against a planted adapter whose only test names a well-formed document.
...
**Addendum, 2026-08-21, and it reverses one closure above.** Step 1 shipped at
0.30.0.0 (T55). Step 2 ran today, both probes in one window so that a
difference between them could not be a difference between hours:

| Probe, 09:15-09:28 UTC | Result |
| --- | --- |
| 600 ICMP packets to the channel's address | **0% loss**, rtt 22.8 ±0.3 ms - at 99.8% power against a 1% rate |
| 600 TCP requests, one per second, same window | **64 failed, 10.7%** |
| The collector, 33 s cadence, same window | **0 refusals in 34 polls** |

Generic packet loss on the IPv6 path is **closed**, at the power this entry
asked for. What remains selects on protocol and on request rate: the probe at
1 s bled and the collector at 33 s did not, through the same window to the
same address. That is the shape of a limiter, and **the rate-limiting closure
above - ten requests in fifty seconds, all 200 - was wrong**: an n=10 probe
has no power against a limiter that meters a longer window, and it was read as
a closure anyway. Reopened. Which side imposes the limit is `[unknown]`; a
window-length estimate needs the same probe at production cadence, which is
hours rather than minutes and is the next step here.

**Step 4 was taken before steps 2 and 3, and this entry said not to.** The
retry shipped at 0.36.0.0 with the cause unknown, and the warning about
masking came true in a stronger form: F110 did not mask the symptom, it
erased the line that reported it. The order survives this entry as its
lesson, not as its sequence.
...
Status: `done` at 0.54.3.0, row written from a reading on `vm-mavo` [tier 1]
...
latency (without them the max is 3,179.6 s and the median does not move); the
window opens before D-027, so the first day and a half was collected at ~141 s;
and there is no row for the API, because `ts_source` there is an alert's
declared start and a four-month-old standing alarm is inheritance rather than
detection, with nothing in the store separating the two.

**The command that produced it.** `mavo latency --store /var/lib/mavo/events
--interval-s 33 --source telegram`. That command exited
...
**2026-08-13 08:59:43**, the drop-in's own mtime; before it the timer fired
every ~141 s (measured: 153 starts in six hours against 655 after). The first
day and a half of the distribution was taken by an instrument polling four
times slower than the one described, so the median is a mixture of two
configurations. The direction is safe - a slower poll only inflates our share
of the wait, so the upstream bound stays a bound - but the figure is not what
its label says until the row says this.
...
| RSO CAP resource | behind a token, per the publisher's integration page; nothing else read | `[measured]` on the token, FEED-SPEC section 2; the resource itself unread |
...
**What a read under a token would make possible.** The first acceptance
bullet below - one real read, committed as a fixture - would become a read of
CAP, and the second is answerable in the same read: `geocode` either carries
a TERYT code or it does not. Neither read is made. The task stays open on
that, and the CAP half is `blocked-external` on the token while the
list-page half is not.
...
**Closed at 0.39.1.0.** The acceptance was a rule, and a rule with nobody to
enforce it is a preference, so it is enforced instead of restated.
`check_the_coverage_floor_stays_a_ratchet` in `tools/docs_audit.py` fails when
`coverage_percent` runs more than five points above `coverage_floor_percent`.
Only that direction: a floor *above* the measurement is `--cov-fail-under`'s
business and catching it here would turn a ratchet into a target, which the
entry warned against and a regression now asserts. Today: 96.43 against 95.
...
Status: `done` at 0.54.1.0 [tier 2]
...
names and pinned counts; nothing checked the rest of the backticked identifiers.

**Acceptance met.** `check_cited_identifiers_exist` in `tools/docs_audit.py`
reads three shapes out of `docs/*.md` and the README - a call written `name()`,
a module-qualified `mavo.x.y`, and a constant in capitals carrying an
underscore - and resolves each against every name bound anywhere under `mavo/`,
...
**One of the six is F55's own constant.** `MAX_ALARMS_PER_WEEK` is cited by
`docs/COMPUTATION.md` and by the defect entry as a name that was removed. A
register of defects that could not name what is gone would be a register of
the present only, which is the opposite of its job.
...
Status: `done` at 0.54.4.0, reproduced then fixed [tier 3]
...
**It reproduced.** Holder at pid 483 in the host namespace, challenger under
`unshare --pid --fork` on the same directory, `os.kill` finding nothing,
challenger taking the lock `[measured 2026-09-10]`. All three acceptance items
follow: exclusion is now `flock` on a descriptor against the inode, MT15 is the
threat-model row it earned by being observed rather than reasoned about, and
`tests/test_t26_directory_lock.py` carries five regressions - **F160**.

**The load-bearing regression needs no containers.** The defect was never
really about namespaces: it was that liveness of a self-reported number was the
whole exclusion. `test_a_dead_pid_in_the_file_does_not_release_a_held_lock`
holds a live lock, overwrites the file with a dead pid, and requires the
refusal to stand - red against every implementation before this one, and
portable to a laptop where `unshare` does not exist. The literal namespace
reproduction is kept beside it and skips where it cannot run, which is why it
is not the one the gate leans on.
...
Status: `decision`, **S7** [tier 2]
*Token corrected at F113.* The line declared `ready` and then said in the same
sentence that this is a decision rather than work. `tools/todo_index.py` read
the sentence and counted it `decision`, which was the right answer reached from
the wrong half of the line; the guard added with F113 reports the two halves
disagreeing rather than picking one. Nothing about the count moves: the entry
was already being counted this way.
...
**2026-08-29, a window offered itself and was declined, on the record.** The
channel fell silent for 9 h 33 m and the pipeline sat in `degraded` - a free
staleness exercise, at first glance. Declined because the consumer's
fresh-to-stale transition had passed hours before anyone was watching:
stopping the push mid-silence would have *extended* a state, and this drill is
about *transitions* read by a human as they happen. Deferral recorded here so
it is a decision with a reason, not the drift it would otherwise be
...
Status: `done` at 0.39.1.0, closed on evidence [tier 2]
...
**Closed at 0.39.1.0, and it was closed before it was closed.** Every clause of
the acceptance already held in the tree: `ENTRY` is `^## (T\\d+[a-z]?)\\.`,
`decision_entries` is `^## (D-\\d+[a-z]?)`, `cited_decisions` is
`\\bD-\\d{3}[a-z]?\\b`, `T8a` and `T8b` are in the generated index and `D-012a`
counts separately `[measured, from the tree]`. The entry sat `ready` through six
releases over work that was finished. Logged as **F118**, with T72's stale file
list as its second instance.
...
**Closed at 0.39.1.0 by D-035**, which records signing as out of scope with
three reopen conditions. One thing in the entry above did **not** carry into the
decision: the claim that Software Heritage and OpenTimestamps already provide an
authorship record. Nothing in this tree establishes that either exists, so a
decision resting on it would be F100's shape again. D-035 rests only on the
measured state of the tags, and the unverified claim becomes its own reopen
condition.


## T64. A claim about a systemd unit quotes the unit
Status: `done` at 0.39.1.0 [tier 2]
...
**Closed at 0.39.1.0.** `check_unit_claims_quote_the_unit` in
`tools/docs_audit.py`, verified red against the 0.32.7.0 text the acceptance
names. Scope rather than an allow-list of sentences: a directive fires only
inside a section that also names one of our units or `/etc/systemd`, because
`Restart=always` in a paragraph explaining systemd is a fact about systemd.

**It found one thing on its first run and it was its own class.** The cadence
section asserted the contents of `mavo-collect.timer.d/interval.conf` with
nothing quoted. The assertion was **true** - the reading matched it to the
character - which is the point: this check refuses an unsourced claim, not a
wrong one. Repaired by quoting `systemctl cat` for both timers and their
drop-ins, which also ends the state where four files deciding how often the
instrument runs existed nowhere in the tree.


## T65. Decide what a successful poll with no state change should render as
Status: `done` at 0.39.1.0 [tier 1]
...
**Closed at 0.39.1.0 by D-034**, on 19,761 `publish.cycle` records:
**20.5% `degraded`**, daily range 13.2 to 32.8 over six full days
`[measured]`. Option 2 chosen and narrowed to one contract field, the age of
the last successful poll, with T71 named as its prerequisite because
`mavo collect` has no sink to read that age from.

Option 3 was refused for a reason the entry could not have known: it is already
implemented, by the consumer, as a heuristic on its own render age, and that
heuristic failed silently for a fortnight under F116. **F107 closes with this.**
...
It has no `ThreatSource.poll` and will not get one: a communique is not an
alert transition. **From 0.55.0.0 it has a place in the pipeline that is its
own** (D-053): `mavo rso` records what each address served (D-054),
`mavo-rso.timer` is the unit the 0.55.0.0 install writes, and `mavo/poland.py`
composes `pl_warnings` from those rows. It has had a caller since before this
entry said otherwise (F166).

**What remains.** The table of its own exists (`communiques`, 0.38.0.0). A page
fetched over the wire *on the host*, which is now T85's first reading rather than
a separate chore; and backfill as far as the endpoint pages, which nothing has
started.
...
a repaired rate between 1.1% and 3.7% `[inference]`. The failures are
independent at a one-second timescale or they are not, and seven trials cannot
tell 100% from 70%.

**Acceptance:** the refusal rate over at least 24 hours on the host after this
...
| Deploy of 0.36.0.1 | **2026-08-21 08:52:36 UTC** |
| Read no earlier than | 2026-08-22 08:52 UTC |
| Refusal path re-verified | 08:52-09:09: 31 starts, 31 finishes, 0 refusals, no exits - the three counts agree, which under F110 they could not |
| **The binding reading** | 2026-08-22 08:55:17 UTC, window 24 h 2 m [measured] |

**Acceptance met, 2026-08-22.** Starting 2628 = Finished 2604 + Failed 24, and
Failed 24 = UNREACHABLE 24: exact three-way agreement, so the token is alive
and the zero-shaped failure F110 produced is excluded. **Refusal rate 24/2628
= 0.9%**, against the 4% threshold. T69 closes; F109 stays closed. An earlier
read at 23 h 49 m returned the same rate and was discarded for being eleven
minutes short of the window, because a window rounded up is not a window.

**The mechanism measurement contradicts the estimate this task shipped on.**
`2 attempts` counted 23, so the retry rescued 23 of 47 first-attempt refusals:
**49% retry success, 95% CI roughly 34-64%** [inference], which excludes the
65% one-sided lower bound the seven-observation sample gave. Refusals are
correlated at the one-second timescale: an endpoint that refused is inclined
to refuse again. The retry **halves** the refusal rate (1.8% to 0.9%) rather
than eliminating it, which is worth having and is not what 7/7 suggested.
Seven trials could not tell 100% from 70%; forty-seven can.
...
`time_connect` is zero on every refusal and no bytes arrive, so a SYN leaves
and nothing answers. Three explanations are excluded by measurement: DNS (40
probes with resolution skipped refused at the same rate), the absent IPv4
route (20 consecutive fetches all chose the AAAA record), and path MTU (a
black hole there completes the handshake and stalls on transfer, and these
never reach the handshake).

What remains is loss on the IPv6 egress path or refusal at the far end, and
nothing in the journal distinguishes them, because the application sees one
duration either way.

**Tier 3, deliberately.** T69's repair works whichever answer this has, so
knowing costs a diagnosis and buys nothing operational today. It matters if
T69's acceptance fails, and then it is the next question rather than a
parallel one.

**Acceptance:** `tcpdump -i ens4 ip6 and port 443` across at least one
refusal, with the SYN retransmissions counted and any response recorded.
Refusal at the far end and loss on the path look different on the wire even
...
figure this week's findings rest on - attempts, refusals, latencies, F110's
168 silent deaths - was greppable prose in a journal that rotates on the
system's schedule, not ours, and the counting rules had to be rediscovered
twice (F109's denominator, F110's missing token).

**What this buys, stated from this week rather than in general:** a
`collect.attempt` record with outcome, duration and attempt count would have
made F109 a one-line query, made F110 visible within an hour instead of
fourteen, and would give T69's acceptance a denominator that is a field
rather than a subtraction of two greps.

**Scope:** `sink_from_environment()` in `_cmd_collect`, one record kind, no
new module. `MAVO_LOG_FILE` already sits on `mavo-collect.service` - the
variable has waited for a reader since F103.

**Acceptance:** one poll on the host produces one `collect.attempt` record,
and the three journald counts in `docs/DEPLOYMENT.md` reconcile against the
sink's own totals over the same window.


## T72. Warnings are not errors in the suite, and the suite leaks handles
Status: `done` at 0.39.1.0 [tier 3]

`filterwarnings = error` is absent from `pyproject.toml`. Adding it today
fails the suite on unclosed file handles in `tests/test_obs.py` and
`tests/test_store.py` - about fifteen sites - so the flag documents a debt the
suite currently carries rather than a switch to flip. Deferred knowingly at
0.33.0.2 and recorded here so the deferral has a number; the handles are the
work, the flag is the receipt.

**Acceptance:** `filterwarnings = ["error"]` in `pyproject.toml`, gate green.

**Closed at 0.39.1.0.** `filterwarnings = ["error"]` is in `pyproject.toml` and
the gate is green. The work was four sites writing `with sqlite3.connect(...)`,
which commits and does not close: `tests/test_rso.py`, `tests/test_latency.py`,
`tests/test_sprint8.py` and `tools/latency.py`. **Neither file this entry named
was among them**, and the fourth is not a test, so the entry's own title was
narrower than its subject. Logged as **F119**, with the stale file list as the
second instance of **F118**.

## T73. The delivery cadence has no record in this project's own sink
Status: `ready` [tier 1], follows F116

`run.jsonl` carries three event kinds and none of them names a delivery
`[measured, 39,524 lines: `publish.interval`, `publish.cycle`, `sink.opened`].
Every figure F116 rests on came from `sshd` in journald on the *consumer's*
host, which records the channel by accident and under a retention nobody has
measured on either machine.

**Tier 1 because it is the last stage before a reader and the only one with no
instrument.** T71 is the same shape at the collect stage, where the cost is
diagnostic; here the cost is that a four-fold cadence error ran for weeks and
was found by reading somebody else's log.

**Acceptance:** one delivery on the host produces one record in `run.jsonl` with
its outcome and the target, and the gap distribution F116 leaves outstanding is
computed from that record rather than from `sshd`.

## T74. The delivery timer describes a cadence it stopped running
Status: `ready` [tier 3], follows F116

`mavo-push.timer` still carries `Description=push state.json every two minutes`
while `OnUnitActiveSec` is 30 `[measured, systemctl cat, 2026-08-24]`.
`mavo-collect.timer` carries a `description.conf` drop-in for exactly this and
the delivery timer never got one - including from the release that changed its
cadence, which is F116's own mechanism reappearing inside F116's repair.

**Acceptance:** the description matches the effective interval, read back with
`systemctl cat` rather than asserted, and `docs/DEPLOYMENT.md` quotes the
reading.
...
entirely and `feed.conf` contributes nothing `[measured, systemctl cat,
2026-08-24]`. The effective command is the one in `interval.conf`, which happens
to carry `--feed` as well, so **nothing is broken and the file is a decoy**: a
reader looking for where `--feed` comes from finds it in a file that does not
supply it.

**Acceptance:** one drop-in, or two that do not both reset the same directive,
...
matrix again over an identical tree. Measured on `v0.39.0.0`: two runs, 38 s and
40 s, same commit `[measured, from the run list]`. The consumer refuses this
deliberately under D-S57 and this repository does not, which makes the
difference an accident rather than a position.

**This repository is public, so the minutes are free**, and that is the reason
it survived rather than a reason it is right: a second reading of the same bytes
tells a reader nothing and appears in the history as if it did.

**Acceptance:** either the trigger names branches, or a decision records why a
tag run is worth taking here and not next door.

## T77. The false-precision counter counts version numbers
Status: `done` at 0.54.3.0 [tier 3]

`FIGURE` in `tools/precision_lint.py` excludes version strings, and the
exclusion is `(?!\.\d)`, which only reaches the four-segment form this
repository uses for itself. Two ceilings were raised at 0.39.1.0 for figures
that are not measurements: `0.37.x` and `0.38.x` in prose, and `3.12` and
`3.14` naming the interpreters a defect appeared and disappeared on.

**A ceiling that rises for reasons the module is not about stops measuring the
thing it was built for.** Each raise carries a written reason, so the record is
honest and the trend is still upward for the wrong cause.

**Acceptance:** either the pattern excludes an interpreter or truncated version
...
**Closed at 0.54.3.0 on a third answer, and the entry's two branches were both
wrong.** Excluding by shape catches a genuine two-decimal measurement, which is
precisely the figure this counter exists to find; counting interpreter tokens deliberately is what made
two ceilings rise at 0.39.1.0 for reasons the module is not about. The tree
excludes interpreters by an explicit list of names, document versions and
section numbers by context, and four-part versions by shape. **D-052** records
that, `test_an_interpreter_token_is_not_a_figure` is the red and green the
acceptance named, and the cost of naming rather than shaping is paid by
`test_the_exclusion_covers_every_interpreter_the_matrix_names`, which reads the
CI matrix and fails when an interpreter runs there and is not on the list. No
ceiling rose to close this; two fell earlier when the exclusions landed.
...
on 2026-08-29 after `attempts.py` was found unrunnable on the host that holds
the store, and `attempts.py` was moved. `tools/latency.py` reads the store too,
by the same `sqlite3` connection to the same file, and was left where it was.
...
**Closed at 0.52.0.0, and one value wider than the entry asked.** A type
string outside `_KIND` is still folded into UNKNOWN - a type this vocabulary
cannot read is not the absence of an alert - but the adapter now names it and
the regions it arrived on, the recap prints both, and the type strings land in
`feed_attempts.detail` beside the attempt so drift is read off the table and
not off a journal. The wider part: `INFO`, which the Home Assistant
integration D-029 cites lists beside the five categories D-029 recorded, is an
informational message and not an alert, and until this release it would have
raised an ACTIVE of unstated kind on the map. It is now left out of the
snapshot and counted by region. Whether this key ever returns it is
`[unmeasured]`: `tools/api_kind_compare.py` prints the returned vocabulary, and
one run on the host settles it either way.
...
**Origin, measured on 2026-08-29:** the channel served no new message between
04:55:28 and at least 14:40 UTC - 9 h 33 m of verified silence (curl from two
networks and the collector's own transport all read the same page, top post
`334744`), against a corpus mean of 514 posts a day. The pipeline reported it
correctly (`degraded`, `observation_age_s` 34,214). What nobody can currently
say is whether nine hours of silence is the tail of this channel's ordinary
distribution or an event, and "the channel is quiet" will keep being read off
the state page without that context.

**Acceptance:** the distribution of gaps between consecutive `ts_source`
values, from the host store (18 days) and from the corpus (99 nights),
reported as count, p50, p95, p99 and max, with 34,214 s placed in it; landed
beside the arrival-rate material in `docs/CHANNEL.md` with provenance labels.
The two datasets are reported separately, not pooled: the corpus is raid
nights by construction and the store is whatever the timer saw.

**Why tier 2 and not 3:** D-034's field, when it ships, will be read against
exactly this question - "degraded because the source is quiet" needs a base
rate for quiet, or the word carries alarm it has not earned.

## T85. The Polish pipes on the host, read rather than declared
Status: `ready` [tier 1]

0.55.0.0 moves RSO and the PAŻP plan into this package (D-053) and ships four
claims about the host that no reading supports yet: that `mavo-rso.timer` runs
every 900 s and `mavo-airspace.timer` every 300 s (declared in
`mavo/liveness.py` and in `docs/DEPLOYMENT.md`, written before either unit
exists); that the first open by the new version creates three recorded tables
and says so once; that the consumer stands its own readings down on the keys
(read from `mavo-site` 4.76.0.0's `server.py`, never observed on the wire); and
that `pl_airspace.features` keeps `state.json` small enough to push every thirty
seconds, which rests on one body measured by the consumer on 2026-09-14.
...
`state.json` at the reading of that week with the most structures drawn; and,
reported from the consumer's session, the areas page showing the producer's
blocks (`data-state` on `#rcb` and `#airspace`) with the consumer's own timers
still running beside them, which is the handover observed rather than read.

**The install half is met**, read on the host on 2026-09-21 and recorded at
0.55.3.0 (`docs/DEPLOYMENT.md`, the 0.55.0.0 section and Appendix A). Both
units are quoted in place of the plan, with five lines more than it asked for;
`mavo/liveness.py` calls both cadences measured; the first `snapshot=changed`
lines came from both readers at 18:38:28 on 2026-09-19; the three
`[STORE-MIGRATED] created ...` lines are in the report unit's journal alone,
one second after `pip`; and both feeds have read `delivering` since their
first read. The areas page showing the producer's block beside the consumer's
own reader was observed on the wire the same morning for `#airspace`
(`data-state="active"`, 91 activated, the tiled map's own reader reporting the
same 91) and not yet for `#rcb`. **The week half is not met**: rows per day
over seven days, the growth in bytes over a week and the largest `state.json`
of that week need a reading taken after 2026-09-26 18:36 UTC. The first forty
hours are in Appendix A, and they already say where the bytes go: 130
snapshot rows, and a store growing 9.3 MB a day against 1.4 before, most of it
`communiques`.

**Not this task.** Pointing the consumer's `/airspace.json` at
`pl_airspace.features` and removing its two readers are the consumer's release.

## T86. How RCB ends an alert, measured over a week of recorded rows
Status: `ready` [tier 1]

D-055 rests on one pair. The store now keeps every `ogolne` communique and
every list the page served, so the rule can be re-run over a week without a
second guess. **Acceptance.** For every threat recorded in T85's week: whether
an all-clear followed, after how long, naming the same voivodeships or a
subset; every communique `classify` calls an all-clear, read by a person
against its text; every threat that reached `valid_to` unended, read for the
sentence that should have ended it; and the `ALL_CLEAR_*` terms revised from
that reading, with the count of rows each term matched beside it. A threat the
publisher ended in words this rule does not recognise reopens D-055.

## T87. The primary collector and the delivery unit run unconfined
Status: `decision` [tier 2]

Read on the host on 2026-09-21 (`docs/DEPLOYMENT.md`, Appendix A). Four of this
package's units carry the same five confinement lines: no new privileges, a
private `/tmp`, the system read-only, home directories hidden, and
`/var/lib/mavo` as the one writable path. `systemd-analyze security` scores
them 8.3. `mavo-collect-api.service`, which reads the API key and has been the
primary source since D-040, carries none of them and scores 9.2, and so does
`mavo-push.service`, which carries the key to the site host. The ADS-B sampler
beside them scores 1.5, so the host already holds a worked example.

**The decision.** Whether the two units take the same five lines or the
tighter set the sampler's unit shows. Neither changes what the units do; both
are edits on the host that this repository can only quote, and each costs one
restart window and a reading after it.

**Acceptance.** Both units quoted in `docs/DEPLOYMENT.md` after the edit;
`systemd-analyze security` read before and after; and one cycle of each
observed completing under the new unit, which for the API collector is a row
in `feed_attempts` and for delivery the site's `state.json` staying inside its
age ceiling.

## T88. The host reading, as a `mavo` subcommand
Status: `decision` [tier 3]

The reading behind `docs/DEPLOYMENT.md` Appendix A was taken by a session
script, named there by its sha256, staged on the host and not in this tree. It
cannot live in `tools/`: it opens the store, D-038 ships such an instrument as
a `mavo` subcommand, and `tests/lint_domain.py` refuses the alternative. The
freshness check forces a reading at least every fourteen days, so the question
is whether a repeatable command is worth its tests: the reading calls
`systemctl`, `journalctl` and `systemd-analyze`, and the coverage floor holds
only if each of them is faked.

**Acceptance, if taken.** One invocation on the host producing the appendix's
